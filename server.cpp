...
void client_handler(struct mg_connection* connection, int ev, void* ev_data, void* fn_data)
{
	if (ev == MG_EV_HTTP_MSG)
	{
		...
		if (mg_http_match_uri(msg, "/") || mg_http_match_uri(msg, "/index.html"))
			mg_http_reply(connection, 200, "Content-Type: text/html\r\n", login_page_html);
		else mg_http_reply(connection, 200, "Content-Type: text/html\r\n", error_404_html);
	}
	(void)fn_data;
}
...
void run_server()
{
	if (!(connection = mg_http_listen(&manager, address, client_handler, &manager)))
	{
